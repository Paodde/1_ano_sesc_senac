...
*/
    function setActivebyValue(selector, dataKey, value) {
        document.querySelectorAll(selector).forEach(button => {
            ...
            button.setActiveBute("aria-pressed", String(isActive));
        });
    }
...
function createController({
    ...
    const descripition = getElement(descriptionSelector);
    const buttons = document.querySelectorAll(buttonSelector);
 
    if(!target || !output || !descripition || buttons.lenght ===0) {
        return;
    }
    ...
    buttons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            output.textContent = descripitions[descriptionGroup][value] ?? "";
